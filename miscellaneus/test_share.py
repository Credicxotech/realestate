...
def post_the_post(browser): 
    ...
    try:
        ...
        data.update({"flow": {
                ...
                "end_time": end_flow,
                "used_ip": ip,
                "error": "602-Fetching Groups's URLs failed"
                }})
        with open(r"./callback_json/share_post_callback.json",'w') as cb:
            json.dump(data,cb)
        browser.quit()
    ...
    try:
        ...
        data_ = {"Error":"Callback url responded abnormally first time"} 
        r = requests.post(cb_url, json=data_)
    ...
    for i in range(1,len(which_groups)):
        ...
        for k in which_groups_sec:
            grps_sec.append(k.text[:-13])
        for k in range(len(grps_sec)):
            print(grps_sec[k])
            info(grps_sec[k]) 
        ...
        try:    
            ...
            data['trace']['steps'].append({
              'start_time':start_time_sec,
              'end_time': datetime.now().isoformat(' ','seconds'), 
              'action' : 'Clicking on group "{}"'.format(grps_sec[i]),
              'status' : True
            })
        except:
            end_flow = datetime.now().isoformat(' ','seconds')
            error("Could not click on group '{}'".format(grps_sec[i]))  
            print("Could not click on group '{}' , might get blocked".format(grps_sec[i]))  
            data['trace']['steps'].append({
                'start_time':start_time_sec,
              'end_time': datetime.now().isoformat(' ','seconds'), 
              'action' : 'Could not click on group "{}"'.format(grps_sec[k]),
              'status' : False
            })
            data.update({"flow": {
                "uuid": uuid,
                ...
                "end_time": end_flow,
                "used_ip": ip,
                "error": "602-Could not click on group '{}', might get blocked".format(grps_sec[k])
                }})
            with open(r"./callback_json/share_post_callback.json",'w') as cb:
                json.dump(data,cb)
            browser.quit()
        ...
        try:   
            post = WebDriverWait(browser,10).until(EC.presence_of_element_located((By.XPATH,"//span[contains(text(),'Post')]")))
            post.click()    
            print("### Post shared to group {}".format(grps_sec[i]))
            info("Post shared to group {}".format(grps_sec[i]))
            data['trace']['steps'].append({
              'start_time':start_time_sec,
              'end_time': datetime.now().isoformat(' ','seconds'), 
              'action' : 'Posting',
